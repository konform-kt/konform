...
internal class ValidationBuilderImpl<T> : ValidationBuilder<T>() {
    companion object {
        ...
        private data class SingleMethodPropKey<T,R> (
            val method: KFunction1<T,R>,
            val modifier: PropModifier
        ) : PropKey<T>() {
            override fun build(builder: ValidationBuilderImpl<*>): Validation<T> {
                @Suppress("UNCHECKED_CAST")
                val validations = (builder as ValidationBuilderImpl<R>).build()
                return when(modifier) {
                    NonNull -> NonNullMethodValidation(method,validations)
                    Optional -> OptionalMethodValidation(method,validations)
                    OptionalRequired -> RequiredMethodValidation(method,validations)
                }
            }
        }
        ...
        private data class IterableMethodKey<T, R>(
            val method: KFunction1<T, Iterable<R>>,
            val modifier: PropModifier
        ) : PropKey<T>() {
            override fun build(builder: ValidationBuilderImpl<*>): Validation<T> {
                @Suppress("UNCHECKED_CAST")
                ...
                @Suppress("UNCHECKED_CAST")
                return when (modifier) {
                    NonNull -> NonNullMethodValidation(method, IterableValidation(validations))
                    Optional -> OptionalMethodValidation(method, IterableValidation(validations))
                    OptionalRequired -> RequiredMethodValidation(method, IterableValidation(validations))
                }
            }
        }
        ...
        private data class ArrayMethodKey<T, R>(
            val method: KFunction1<T, Array<R>>,
            val modifier: PropModifier
        ) : PropKey<T>() {
            override fun build(builder: ValidationBuilderImpl<*>): Validation<T> {
                @Suppress("UNCHECKED_CAST")
                ...
                @Suppress("UNCHECKED_CAST")
                return when (modifier) {
                    NonNull -> NonNullMethodValidation(method, ArrayValidation(validations))
                    Optional -> OptionalMethodValidation(method, ArrayValidation(validations))
                    OptionalRequired -> RequiredMethodValidation(method, ArrayValidation(validations))
                }
            }
        }
        ...
        private data class MapMethodKey<T, K, V>(
            val method: KFunction1<T, Map<K, V>>,
            val modifier: PropModifier
        ) : PropKey<T>() {
            override fun build(builder: ValidationBuilderImpl<*>): Validation<T> {
                @Suppress("UNCHECKED_CAST")
                val validations = (builder as ValidationBuilderImpl<Map.Entry<K, V>>).build()
                return when (modifier) {
                    NonNull -> NonNullMethodValidation(method, MapValidation(validations))
                    Optional -> OptionalMethodValidation(method, MapValidation(validations))
                    OptionalRequired -> RequiredMethodValidation(method, MapValidation(validations))
                }
            }
        }
    }
    ...
    private fun <R> KFunction1<T, R?>.getOrCreateBuilder(modifier: PropModifier): ValidationBuilder<R> {
        val key = SingleMethodPropKey(this, modifier)
        @Suppress("UNCHECKED_CAST")
        return (subValidations.getOrPut(key) { ValidationBuilderImpl<R>() } as ValidationBuilder<R>)
    }
    ...
    private fun <R> KFunction1<T, Iterable<R>>.getOrCreateIterablePropertyBuilder(modifier: PropModifier): ValidationBuilder<R> {
        val key = IterableMethodKey(this, modifier)
        @Suppress("UNCHECKED_CAST")
        return (subValidations.getOrPut(key) { ValidationBuilderImpl<R>() } as ValidationBuilder<R>)
    }
    ...
    override fun <R> onEachIterable(prop: KFunction1<T, Iterable<R>>, init: ValidationBuilder<R>.() -> Unit) {
    ...
    override fun <R> onEachArray(method: KFunction1<T, Array<R>>, init: ValidationBuilder<R>.() -> Unit) {
        ArrayMethodKey(method, NonNull).getOrCreateBuilder<R>().also(init)
    }
    ...
    override fun <K, V> onEachMap(method: KFunction1<T, Map<K, V>>, init: ValidationBuilder<Entry<K, V>>.() -> Unit) {
        MapMethodKey(method, NonNull).getOrCreateBuilder<Map.Entry<K, V>>().also(init)
    }
    ...
    override fun <R> KFunction1<T, R?>.ifPresent(init: ValidationBuilder<R>.() -> Unit) {
        getOrCreateBuilder(Optional).also(init)
    }
    ...
    override fun <R> KFunction1<T, R?>.required(init: ValidationBuilder<R>.() -> Unit) {
        getOrCreateBuilder(OptionalRequired).also(init)
    }
    ...
    override fun <R> KFunction1<T, R>.invoke(init: ValidationBuilder<R>.() -> Unit) {
        getOrCreateBuilder(NonNull).also(init)
    }
}
