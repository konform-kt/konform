...
internal class ValidationBuilderImpl<T> : ValidationBuilder<T>() {
    ...
    private fun <R> ((T) -> R?).getOrCreateBuilder(name: String, modifier: PropModifier): ValidationBuilder<R> {
    ...
    private fun <R> ((T) -> Iterable<R>).getOrCreateIterablePropertyBuilder(name: String, modifier: PropModifier): ValidationBuilder<R> {
    ...
    override fun <R> ((T) -> R).invoke(name: String, init: ValidationBuilder<R>.() -> Unit) {
        getOrCreateBuilder(name, NonNull).also(init)
    }

    override fun <R> onEachIterable(name: String, prop: (T) -> Iterable<R>, init: ValidationBuilder<R>.() -> Unit) {
        prop.getOrCreateIterablePropertyBuilder(name, NonNull).also(init)
    }

    override fun <R> onEachArray(name: String, prop: (T) -> Array<R>, init: ValidationBuilder<R>.() -> Unit) {
        ArrayPropKey(prop, name, NonNull).getOrCreateBuilder<R>().also(init)
    }

    override fun <K, V> onEachMap(name: String, prop: (T) -> Map<K, V>, init: ValidationBuilder<Entry<K, V>>.() -> Unit) {
        MapPropKey(prop, name, NonNull).getOrCreateBuilder<Map.Entry<K, V>>().also(init)
    }

    override fun <R> ((T) -> R?).ifPresent(name: String, init: ValidationBuilder<R>.() -> Unit) {
        getOrCreateBuilder(name, Optional).also(init)
    }

    override fun <R> ((T) -> R?).required(name: String, init: ValidationBuilder<R>.() -> Unit) {
